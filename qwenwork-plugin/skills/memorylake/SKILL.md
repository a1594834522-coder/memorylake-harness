...
other QwenWork machines, Claude Code, Codex, WorkBuddy, and more. Any memory
QwenWork keeps itself only covers this app; keep using it exactly as your
instructions say. MemoryLake is where everything else lives.

## The contract: search first

Every user message arrives with a reminder to search MemoryLake before
replying. Make the search your first action of the turn, before other tools.
Skip it only for pure small talk with nothing to look up.

## Searching

Run `ml-recall` with your shell tool, by the absolute path the reminder gives
(normally `~/.qwenworkcn/plugins-custom/memorylake/bin/ml-recall`):

```bash
"$HOME/.qwenworkcn/plugins-custom/memorylake/bin/ml-recall" "user's preferred editor"
"$HOME/.qwenworkcn/plugins-custom/memorylake/bin/ml-recall" "Q4 revenue figures" --top-k 10
```
...
The search-first reminder has its own switch in the same file:
`recall_reminder: false`.
