#!/usr/bin/env bash
# Shared helpers for the memorylake QwenWork plugin.
#
...
# configures them all. The QwenWork-specific pieces (per-session turn state,
# prompt cleanup, tool classification) are at the bottom. Logic twin of
# workbuddy-plugin/scripts/lib/common.sh: the two hosts run the same
# Claude-Code-style hooks, so a fix to one usually belongs in both.
#
# Every hook sources this and then calls ml_load_config. The load bails out as
# early as possible: the cost of a session with no MemoryLake configured must
...
ml_load_config() {
  ...
  # Keys shared by the WorkBuddy and QwenWork plugins (one setting governs
  # both hosts on a machine). recall_reminder is lenient (absent
  ...
  export ML_STATUS_LINE ML_RECALL_REMINDER ML_SYNC_CONVERSATIONS
  return 0
}
...
# location (populated by the memorylake-init skill when the user opts in to a managed
...
ml_cli() {
  ...
  # The CLI installer's default location. A desktop app's hooks may run with
  # a PATH that never saw the shell profile that adds it.
  if [ -x "$HOME/.local/bin/memorylake" ]; then
    printf '%s' "$HOME/.local/bin/memorylake"
    return 0
  fi
  return 1
}
...
# ---------- QwenWork session state ----------------------------------------------
#
# One directory per QwenWork session under state/qwenwork/<session>/:
#
#   turn.json      the current turn's id; keys the turn's synced messages
...
#
# QwenWork's engine (Qoder CLI) hands every hook the session's session_id. A
# subagent's hooks carry the SAME session_id plus agent_id/agent_type, so
# "is this the main agent" is decided by agent_id, not by the session.
...
ml_qw_session_dir() {
  printf '%s/qwenwork/%s' "$(ml_state_dir)" "$(ml_qw_safe_id "$1")"
}

# This plugin's installed root (~/.qwenworkcn/plugins-custom/memorylake).
#
# Hooks get it as CLAUDE_PLUGIN_ROOT (Qoder sets it next to QODER_PLUGIN_ROOT);
# the directory of the running script is the fallback.
ml_qw_plugin_root() {
  if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ]; then
    printf '%s' "$CLAUDE_PLUGIN_ROOT"
  elif [ -n "${QODER_PLUGIN_ROOT:-}" ]; then
    printf '%s' "$QODER_PLUGIN_ROOT"
  else
    (cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
  fi
}

# The command the model should run to search. An absolute path: whether
# QwenWork puts a plugin's bin/ on the model's PATH is not established, and a
# bare "ml-recall" that is not found would read to the model like an empty
# memory.
ml_qw_recall_cmd() {
  printf '%s/bin/ml-recall' "$(ml_qw_plugin_root)"
}

# The user's words with the host's own injections removed.
#
# QwenWork prepends <system-reminder> blocks to a user message (workspace
# info, time, directory access); only what is left is conversation. The
# <user_query> wrapper is honoured too, in case the host (or a WorkBuddy
# import) sends the user's text wrapped that way.
ml_qw_clean_prompt() {
  perl -0777 -ne '
