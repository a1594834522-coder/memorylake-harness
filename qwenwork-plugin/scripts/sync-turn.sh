...
#
# One MemoryLake conversation per QwenWork session (custom id
# qwenwork:<session_id>); the server distills memories from it, so nothing
...
#     ml_qw_clean_prompt) and the assistant's final reply. Tool calls, tool
...
# in sync.json for the memorylake-status skill. The hook always exits 0 -- on Stop, a
...
# Detach. hooks.json already marks this hook "async": true, which Qoder's
# hook schema accepts. This first stdout line is the other half of the same
# protocol -- a hook announcing itself async -- and is kept as a fallback: in
# WorkBuddy, which runs the same hook model, only this line worked, and a sync
# that blocks the end of every turn (2.7 to 16.6 s there) is not an option.
printf '%s\n' '{"async":true,"asyncTimeout":180000}'

cwd=$(printf '%s' "$input" | jq -r '.cwd // empty' 2>/dev/null)
session_id=$(printf '%s' "$input" | jq -r '.session_id // empty' 2>/dev/null)

# The main agent's turn only (see common.sh on agent_id).
...
if [ -n "$turn_id" ] && [ -n "$reply" ] \
    && ! cat "$outbox" "$draining" 2>/dev/null | grep -qF "\"qw:${turn_id}:assistant\""; then
  jq -cn \
    --arg id "qw:${turn_id}:assistant" \
    --arg text "$(printf '%s' "$reply" | ml_qw_clip 8000)" \
...
assistant_actor=""
[ -f "$ws_dir/qwenwork-assistant-actor" ] && assistant_actor=$(cat "$ws_dir/qwenwork-assistant-actor" 2>/dev/null)
if [ -z "$assistant_actor" ]; then
  assistant_actor=$("$CLI" actor get --by-custom-id qwenwork-agent 2>/dev/null | jq -r '.id // empty' 2>/dev/null)
  if [ -z "$assistant_actor" ]; then
    assistant_actor=$("$CLI" actor create --custom-id qwenwork-agent --display-name QwenWork \
        --type ASSISTANT 2>/dev/null | jq -r '.id // empty' 2>/dev/null)
  fi
  [ -n "$assistant_actor" ] || { record error "could not resolve or create the QwenWork assistant actor"; exit 0; }
  ...
  printf '%s' "$assistant_actor" >"$ws_dir/qwenwork-assistant-actor" 2>/dev/null
fi
...
if [ -z "$conversation_id" ]; then
  conv_cid="qwenwork:$(ml_qw_safe_id "$session_id")"
...
if [ -z "$conversation_id" ]; then
  ...
  conversation_id=$("$CLI" conversation create --workspace "$ML_WORKSPACE" \
      ...
      --name "QwenWork: ${title:-session ${session_id:0:8}}" \
      --metadata harness=qwenwork --metadata "session=${session_id}" 2>/dev/null \
    | jq -r '.id // empty' 2>/dev/null)
...
while IFS= read -r line; do
  ...
  if err=$("$CLI" conversation message append --workspace "$ML_WORKSPACE" \
      ...
      --metadata "role=${side}" --metadata harness=qwenwork \
