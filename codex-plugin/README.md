...
one client**. This plugin connects it to MemoryLake: session summaries flow up
...
Then set up MemoryLake itself — just ask Codex:
...
per-repo MemoryLake project the Claude Code harness uses**, so both
...
**Status.** One line at session start reporting whether MemoryLake is
...
- With `sync_on_write: true`, Codex session summaries — which describe what
  you worked on — are uploaded to your MemoryLake workspace after each turn.
