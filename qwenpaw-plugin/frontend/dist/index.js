const e = window.QwenPaw.host.React, { useState: g, useCallback: B, useEffect: F, useMemo: E } = e, {
  ...
  en: {
    ...
    syncHelp: "Record this Agent's conversations in MemoryLake so it can distill memories from them. Only the text of what the user says and what the Agent replies is sent, in batches after every N user turns. Tool calls, tool results, and reasoning never leave this machine.",
  ...
  zh: {
    title: "MemoryLake",
    intro: "跨项目、跨设备、跨客户端的长期记忆。如果这台机器已经为 Claude Code、Codex、dsh 或 opencode 配置过 MemoryLake，所有字段留空即可，会直接使用共享的 ~/.memorylake 配置和登录。",
    region: "部署站点",
    ...
    sync: "对话同步",
    syncHelp: "把该 Agent 的对话记录到 MemoryLake，由它在后台提炼记忆。只发送用户说的话和 Agent 回复的文字，每 N 轮用户对话批量发送一次。工具调用、工具结果和推理过程不会离开本机。",
